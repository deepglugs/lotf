# Cleric Class Guide

**Version:** 0.3.6

The Cleric is a Wisdom-based divine caster focused on healing, radiation damage, and sustain. They excel in longer fights where their passive regeneration and party healing can turn the tide.

---

## Level Features

These are automatically granted at the listed level.

| Level | Icon | Feature |
|-------|------|---------|
| 1 | ![Heal](assets/heal_icon.webp) | Heal |
| 2 | ![Divine Grace](assets/divine_grace_icon.webp) | Divine Grace |
| 3 | ![Channel Divinity](assets/channel_divinity_icon.webp) | Channel Divinity |
| 5 | ![Sacred Aura](assets/sacred_aura_icon.webp) | Sacred Aura |
| 7 | ![Divine Ward](assets/divine_ward_icon.webp) | Divine Ward |
| 9 | ![Blessed Resilience](assets/blessed_resilience_icon.webp) | Blessed Resilience |
| 11 | ![Holy Aura](assets/holy_aura_icon.webp) | Holy Aura |
| 14 | ![Extra Attack](assets/extra_attack_icon.webp) | Extra Attack |
| 18 | ![Divine Ascension](assets/divine_ascension_icon.webp) | Divine Ascension |
| 20 | ![Avatar of Light](assets/avatar_of_light_icon.webp) | Avatar of Light |

### ![](assets/heal_icon.webp) Heal *(L1 — Auto)*
A baseline healing ability granted at level 1.

### ![](assets/divine_grace_icon.webp) Divine Grace *(L2)*
Passive. Adds your **Wisdom modifier** to all saving throws.

### ![](assets/channel_divinity_icon.webp) Channel Divinity *(L3 — 5 Heavenfire energy, once per rest)*
Heal all allies for **2d8 + Wisdom modifier** HP, and deal **2d8 radiation damage** to all enemies simultaneously. Resets on rest.

### ![](assets/sacred_aura_icon.webp) Sacred Aura *(L5)*
Passive. Regenerates **1d4 HP per turn**.

### ![](assets/divine_ward_icon.webp) Divine Ward *(L7)*
Passive. Grants permanent **resistance to radiation damage**.

### ![](assets/blessed_resilience_icon.webp) Blessed Resilience *(L9)*
Passive. Regenerates **1d6 HP per turn** (stacks with Sacred Aura for up to 1d4+1d6 per turn).

### ![](assets/holy_aura_icon.webp) Holy Aura *(L11)*
Passive. Grants **+2 to spell saving throws** and **+2 Wisdom**.

### ![](assets/extra_attack_icon.webp) Extra Attack *(L14)*
Passive. Grants one additional attack per turn.

### ![](assets/divine_ascension_icon.webp) Divine Ascension *(L18)*
Passive. Permanently increases **Wisdom by 4**, with an ability score cap of 24.

### ![](assets/avatar_of_light_icon.webp) Avatar of Light *(L20)*
Passive. Reduces all spell costs by **25%** and adds your **Wisdom modifier as bonus radiation damage** to all spells.

---

## Spells (Level Options)

Spells are chosen on level up and added to your action list.

### Level 1
| Icon | Spell | Energy | Description |
|------|-------|--------|-------------|
| ![](assets/aid_icon.webp) | Aid | 3 (Heavenfire) | Grants target temporary HP equal to **1d6 + Wisdom modifier + 1 per spell level**, lasting until rest. |
| ![](assets/firebolt_icon.webp) | Firebolt | 1 | Standard fire damage spell. |
| ![](assets/slap_icon.webp) | Divine Bitch Slap | 5 (Heavenfire) | Divine melee strike. |

### Level 2
| Icon | Spell | Energy | Description |
|------|-------|--------|-------------|
| ![](assets/sacred_flame_icon.webp) | Sacred Flame | 3 (Heavenfire) | Calls down sacred flame on a target. Deals **Nd8 + Wisdom modifier** radiation damage (N scales as `1 + level/3`). Dexterity save for half. |

### Level 3
| Icon | Spell | Energy | Description |
|------|-------|--------|-------------|
| ![](assets/smite_icon.webp) | Divine Smite | 5 (Heavenfire) | Powerful melee strike with divine damage. |

### Level 5
| Icon | Spell | Energy | Description |
|------|-------|--------|-------------|
| ![](assets/fireball_icon.webp) | Fireball | — | Large area fire damage. |
| ![](assets/greater_heal_icon.webp) | Greater Heal | 6 (Heavenfire) | Heals target for **4d8 + Wisdom modifier x2**. Scaling with level. |
| ![](assets/blessed_strike_icon.webp) | Blessed Strike | 2 (Heavenfire) | Melee weapon hit + self-heal for **1d6 + Wisdom modifier**. |

### Level 7
| Icon | Spell | Energy | Description |
|------|-------|--------|-------------|
| ![](assets/holy_word_icon.webp) | Holy Word | 6 (Heavenfire) | All enemies take **Nd8 + Wisdom modifier** radiation damage (N scales as `2 + level/5`). Enemies who fail a Wisdom save are **Stunned for 1 turn**. |

### Level 9
| Icon | Spell | Energy | Description |
|------|-------|--------|-------------|
| ![](assets/greater_heal_icon.webp) | Mass Heal | 8 (Heavenfire) | Heals all allies (including self) for **3d6 + Wisdom modifier**. |

---

## Affinity Spell Options *(L6+ — Affinity-Gated)*

| Affinity | Icon | Spell | Energy | Description |
|----------|------|-------|--------|-------------|
| Heavenfire | ![](assets/sacred_temptation_icon.webp) | Sacred Temptation | 0 (Lust) | Divine temptation that inflicts Horny. Charisma save to resist. |

---

## Feat Levels

Generic feats are available at levels **4, 8, 12, and 16**.

---

## Tips

- **Wisdom is your primary stat.** It drives Divine Grace (saving throws), Sacred Flame damage, Blessed Strike heals, Holy Word damage, and Avatar of Light. Stack it.
- **Sacred Aura + Blessed Resilience** stack, giving up to **1d4 + 1d6 HP regen per turn** by level 9 — you become very hard to kill in extended fights.
- **Channel Divinity** is a swing ability: heals your whole party while damaging all enemies at once. Use it when the fight is balanced — it can flip the momentum entirely.
- **Holy Word** (L7) is your best crowd-control nuke. It hits every enemy and has a stun chance. Use it when you need to buy a turn.
- **Avatar of Light** (L20) combined with high Wisdom turns all your radiation spells into massive damage. Prioritize Wisdom investment heading into late game.
- The Cleric rewards a patient, reactive playstyle — let passive regen work for you and save high-cost spells like Channel Divinity and Holy Word for pivotal moments.
